...
// ── Users Popup ──────────────────────────────────────
function UsersPopup({ appName, date, color, onClose }) {
  ...
  useEffect(() => {
    ...
    const url = `${BASE}/ZFTX_C_APP_USERS(p_date=${encodedDate},p_app=${encodedApp})/Set?$select=user_id&$count=true&sap-client=100`;

    fetch(url)
      .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(j => { setUsers(j.value || []); setLoading(false); })
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: '400px', maxWidth: '95%', maxHeight: '70vh',
        display: 'flex', flexDirection: 'column',
        boxShadow: '0 12px 40px rgba(0,0,0,0.25)'
      }}>
        {/* Header */}
        <div style={{
          background: color || '#8e44ad', padding: '14px 20px',
          borderRadius: '12px 12px 0 0',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center'
        }}>
          <div>
            <div style={{ color: '#fff', fontWeight: 'bold', fontSize: '14px' }}>👤 Users</div>
        ...
        <div style={{ overflow: 'auto', flex: 1, padding: '12px 0' }}>
          ...
          {error && (
            <div style={{ padding: '16px 20px', color: '#c00', fontSize: '13px' }}>Error: {error}</div>
          )}
        ...
        <div style={{ padding: '10px 20px', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: '#777' }}>{users.length} user(s)</span>
          <button onClick={onClose} style={{
            padding: '6px 18px', background: color || '#8e44ad',
...
function AppActivity({ navigate }) {
  ...
  const [filterUser, setFilterUser]           = useState('');         // NEW: user id input value
  const [userMatchKeys, setUserMatchKeys]     = useState(null);       // NEW: Set of "appName__date" keys that match, null = no filter
  const [userFilterLoading, setUserFilterLoading] = useState(false);  // NEW: spinner while searching
  ...
  // ── NEW: Search all rows for a given user ID ──────────────────────────────
  const searchByUser = async (userId) => {
    // If input is empty, clear filter
    if (!userId.trim()) {
      setUserMatchKeys(null);
      return;
    }

    setUserFilterLoading(true);

    // For every row in data, call the ZFTX_C_APP_USERS API and check if userId exists
    const keys = new Set();

    await Promise.all(
      data.map(async (r) => {
        const appName = r.app_name || r.app_id;
        const date    = r.event_date;
        if (!appName || !date) return;

        try {
          const encodedApp  = encodeURIComponent(`'${appName}'`);
          const encodedDate = encodeURIComponent(date);
          const url = `${BASE}/ZFTX_C_APP_USERS(p_date=${encodedDate},p_app=${encodedApp})/Set?$select=user_id&sap-client=100`;
          const res  = await fetch(url);
          if (!res.ok) return;
          const json = await res.json();

          // Check if any user_id in the result contains the searched text (case-insensitive)
          const found = (json.value || []).some(u =>
            u.user_id?.toLowerCase().includes(userId.toLowerCase())
          );

          if (found) keys.add(`${appName}__${date}`);
        } catch {
  ...
  const filtered = data.filter(r => {
    ...
    // NEW: if userMatchKeys is a Set, only keep rows whose appName__date is in that Set
    const matchUser   = userMatchKeys === null
      ? true
      : userMatchKeys.has(`${r.app_name || r.app_id}__${r.event_date}`);

    const matchDate   = (() => {
      if (!filterDate && !filterDateTo) return true;
      if (!r.last_used_ts) return false;
      const s    = Math.floor(r.last_used_ts).toString();
      if (s.length < 8) return false;
      const tsDate = `${s.slice(0,4)}-${s.slice(4,6)}-${s.slice(6,8)}`;
      const fromOk = filterDate   ? tsDate >= filterDate   : true;
      const toOk   = filterDateTo ? tsDate <= filterDateTo : true;
      return fromOk && toOk;
    })();

    return matchApp && matchSystem && matchDate && matchUser;
  });
  ...
  // ── Clear all filters including user filter ───────────────────────────────
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f0f2f5', overflow: 'hidden' }}>
      ...
      {modal && <Modal title={modal.title} color={modal.color} rows={modal.rows} type={modal.type} loading={modal.loading} onClose={() => setModal(null)} />}
      {usersPopup && <UsersPopup appName={usersPopup.appName} date={usersPopup.date} color="#8e44ad" onClose={() => setUsersPopup(null)} />}
      ...
      <div style={{
        ...
        <div>
          <span style={{ fontWeight: 'bold', fontSize: '18px', color: '#0070f3' }}>App Activity</span>
          <span style={{ fontSize: '12px', color: '#999', marginLeft: '10px' }}>Application usage and launch statistics</span>
        </div>
        <span style={{ marginLeft: 'auto', background: '#0070f3', color: '#fff', padding: '4px 14px', borderRadius: '20px', fontSize: '13px' }}>
          {totalApps} Apps
        </span>
      </div>

      {/* ── Summary Cards ── */}
      <div style={{ display: 'flex', gap: '14px', padding: '14px 20px 0', flexShrink: 0 }}>
        {[
          ...
          <div key={card.id} onClick={() => !loading && openModal(card.id)}
            style={{
              ...
              border: `2px solid ${card.color}`, cursor: 'pointer',
              boxShadow: '0 2px 8px rgba(0,0,0,0.07)',
              transition: 'transform 0.15s'
            }}
            onMouseEnter={e => {
              e.currentTarget.style.transform = 'translateY(-2px)';
              e.currentTarget.style.background = `${card.color}0d`;
            }}
            onMouseLeave={e => {
          ...
          >
            ...
            <div style={{ fontSize: '11px', color: card.color, background: `${card.color}15`, padding: '3px 10px', borderRadius: '10px' }}>
              View →
            </div>
          </div>
        ))}
      ...
      <div style={{
        ...
        alignItems: 'flex-end', flexShrink: 0, flexWrap: 'wrap',
        background: '#fff', margin: '14px 20px 0',
        borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)'
      }}>
        {[
          { label: 'APP TITLE',  width: '160px', value: filterApp,    onChange: e => setFilterApp(e.target.value),    placeholder: 'Search App Title...',  type: 'text' },
          { label: 'SYSTEM',     width: '120px', value: filterSystem, onChange: e => setFilterSystem(e.target.value), placeholder: 'Search System...',     type: 'text' },
          { label: 'FROM DATE',  width: '145px', value: filterDate,   onChange: e => setFilterDate(e.target.value),   placeholder: '',                     type: 'date' },
          { label: 'TO DATE',    width: '145px', value: filterDateTo, onChange: e => setFilterDateTo(e.target.value), placeholder: '',                     type: 'date' },
        ].map(f => (
          <div key={f.label} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <label style={{ fontSize: '11px', color: '#555', fontWeight: 'bold' }}>{f.label}</label>
            <input
              type={f.type}
              style={{ padding: '7px 10px', fontSize: '13px', border: '1px solid #ddd', borderRadius: '5px', outline: 'none', width: f.width }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <label style={{ fontSize: '11px', color: '#555', fontWeight: 'bold' }}>USER ID</label>
          <div style={{ display: 'flex', gap: '4px' }}>
            <input
              type="text"
              style={{ padding: '7px 10px', fontSize: '13px', border: '1px solid #ddd', borderRadius: '5px', outline: 'none', width: '140px' }}
              placeholder="Search User ID..."
              value={filterUser}
              onChange={e => { setFilterUser(e.target.value); if (!e.target.value.trim()) setUserMatchKeys(null); }}
              onKeyDown={e => { if (e.key === 'Enter') searchByUser(filterUser); }}
            />
            <button
              onClick={() => searchByUser(filterUser)}
              disabled={userFilterLoading || loading}
              style={{
                padding: '7px 12px', fontSize: '13px',
                background: '#f0f0f0', color: '#555',
                border: '1px solid #ccc', borderRadius: '5px',
                cursor: userFilterLoading ? 'not-allowed' : 'pointer'
              }}
            >
              {userFilterLoading ? '⏳' : '🔍'}
            </button>
      ...
      {/* ── Bottom Content ── */}
      {!loading && !error && (
        <div style={{ display: 'flex', gap: '16px', padding: '14px 20px 20px', flex: 1, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: '0 0 55%', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', overflow: 'hidden', border: '1px solid #e0e0e0' }}>
            <div style={{ background: '#2b507a', padding: '10px 14px', color: '#fff', fontWeight: 'bold', fontSize: '13px', display: 'flex', justifyContent: 'space-between', flexShrink: 0 }}>
              <span>App Usage — By App Title</span>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                  <tr style={{ background: '#e8f0fe' }}>
                    {['App Title','System','Date','Launches','Users','Last Accessed'].map(col => (
                      <th key={col} style={{
                        padding: '9px 12px',
                ...
                <tbody>
                  {filtered.length === 0
                    ? <tr><td colSpan="6" style={{ textAlign:'center', padding:'40px', color:'#999' }}>
                        {userMatchKeys !== null && userMatchKeys.size === 0
                          ? `No rows found for user "${filterUser}"`
                          : 'No records found'}
                      </td></tr>
                    : filtered.map((r, i) => (
                      <tr key={i} style={{ background: i%2===0 ? '#fff' : '#f9f9f9', borderBottom: '1px solid #eee' }}>
                        <td style={{ padding:'8px 12px' }}>
                          <span style={{ color:'#1d2d3e', fontSize:'11px', fontWeight:'bold' }}>
                            {r.app_name || r.app_id || '-'}
                          </span>
                        </td>
                        <td style={{ padding:'8px 12px' }}>
                          <span style={{ background:'#e8f5e9', color:'#2e7d32', padding:'2px 7px', borderRadius:'4px', fontSize:'11px' }}>
                            {r.system_id}
                          </span>
                        </td>
                        <td style={{ padding:'8px 12px', fontSize:'11px', color:'#555' }}>{r.event_date}</td>
                        <td style={{ padding:'8px 12px', textAlign:'right', fontWeight:'bold', color:'#e74c3c', fontSize:'13px' }}>
                          {r.total_launches?.toLocaleString()}
                        </td>
                        <td
                          style={{ padding:'8px 12px', textAlign:'right', color:'#8e44ad', fontWeight:'bold', fontSize:'13px', cursor:'pointer', textDecoration:'underline' }}
                          onClick={() => setUsersPopup({ appName: r.app_name || r.app_id, date: r.event_date })}
                        >
                          {r.unique_users}
                        </td>
