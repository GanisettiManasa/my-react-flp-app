...
function AppActivity({ navigate }) {
  ...
  const [usersPopup, setUsersPopup]     = useState(null); // { appName, date }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f0f2f5', overflow: 'hidden' }}>
      ...
      {/* Users Popup — zIndex 2000 so it sits above Modal if both open */}
      {usersPopup && (
        <UsersPopup
          appName={usersPopup.appName}
          date={usersPopup.date}
          color="#8e44ad"
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <label style={{ fontSize: '10px', color: '#666', fontWeight: 'bold' }}>APP TITLE</label>
          <input style={{ ...inputStyle, width: '170px' }} placeholder="Search App Title..."
        ...
        <button onClick={() => { setFilterApp(''); setFilterSystem(''); setFilterDate(''); setFilterDateTo(''); }}
                    ...
                    : filtered.map((r, i) => (
                      <tr key={i} style={{ background: i%2===0 ? '#fff' : '#f9f9f9', borderBottom: '1px solid #eee' }}>
                        <td style={{ padding:'8px 12px' }}>
                          <span style={{ color:'#1d2d3e', padding:'2px 7px', borderRadius:'4px', fontSize:'11px', fontWeight:'bold' }}>
                            {r.app_name || r.app_id || '-'}
                          </span>
                        </td>
                        <td style={{ padding:'8px 12px' }}>
                        ...
                        {/* ── Clickable Users cell ── */}
                        <td
                          style={{
                            padding: '8px 12px', textAlign: 'right',
                            color: '#8e44ad', fontWeight: 'bold', fontSize: '13px',
                            cursor: 'pointer', textDecoration: 'underline',
                            userSelect: 'none'
                          }}
                          title={`View users for "${r.app_name || r.app_id}" on ${r.event_date}`}
                          onClick={() => setUsersPopup({
...
//                       <tr key={i} style={{ background: i%2===0 ? '#fff' : '#f9f9f9', borderBottom: '1px solid #eee' }}>
//                         <td style={{ padding:'8px 12px' }}>
//                           <span style={{ color:'#1d2d3e', padding:'2px 7px', borderRadius:'4px', fontSize:'11px', fontWeight:'bold' }}>{r.app_name || r.app_id || '-'}</span>
//                         </td>
//                         <td style={{ padding:'8px 12px' }}>
